...
- **Channel naming** — `mail-{category}` with a short hash suffix appended only on collision
- **Reply confirmation** — checkmark emoji reaction on the user's Slack message (not a new message)
...
3. **Get or create category channel:**
   ...
   - If not found: create private channel `mail-{category}` (append hash on naming collision), invite user, insert row
4. **Get or create thread mapping:**
   - Query `gmail_thread_slack_threads` for `(workspace_id, slack_user_id, gmail_thread_id)`
   - **New thread:** Post parent message to category channel with subject + email body. Store returned `ts` as `slack_thread_ts`.
...
- Bot's own synced messages won't trigger reply handler (`ignoreSelf: true` on Bolt app)
...
| `src/db/repos.ts` | New DB functions: `getOrCreateCategoryChannel`, `getOrCreateThreadMapping`, `getThreadMappingBySlackThread` |
